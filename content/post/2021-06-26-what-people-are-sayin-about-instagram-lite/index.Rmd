...
The main libraries that I will use to do this analysis are `udpipe` for applying the language model used to develop part of speech tagging, `BTM` to construct the Biterm model, and `textrank` / `wordcloud` to do keyword extraction and make the wordcloud. Both `udpipe`, `BTM`, and `textrank` are part of the [Bnosac](http://www.bnosac.de) NLP ecosystem.

The analyses from these posts are heavily inspired from Bnosac's posts on [Biterm Modeling](http://www.bnosac.be/index.php/blog/98-biterm-topic-modelling-for-short-texts) and [Sentiment Analysis](http://www.bnosac.be/index.php/blog/85-you-did-a-sentiment-analysis-with-tidytext-but-you-forgot-to-do-dependency-parsing-to-answer-why-is-something-positive-negative).
...
For data I'll be using the result file from the my [web scraping post](https://jlaw.netlify.app/2021/05/03/scraping-google-play-reviews-with-rselenium/) from April:
...
```{r echo = FALSE}
head(iglite, 3) %>% knitr::kable()
```

## Exploring the IG Lite Review Data

Given the time the initial analysis was run I captured `r nrow(iglite) %>% comma` reviews covering dates from `r paste(range(iglite$dates), collapse = ' and ')`.  However, reviews from earlier than December 2020 are likely referring to the initial version of IG Lite rather than the [relaunched version](https://techcrunch.com/2020/12/16/facebook-launches-new-instagram-lite-app-in-india-global-rollout-to-follow-later/).
...
An alternative way of utilizing the star ratings is to create a Net Promoter-like score.  If you've ever received an email asking "On a scale from 1 to 10 how likely are you to recommend this to a friend", you've been a part of the Net Promoter Score.  [The Net Promoter Score is a score from -100 to 100 that is an index about how willing people are to reccomend a product](https://en.wikipedia.org/wiki/Net_Promoter).  It divides the world into Promoters (scores 9 and 10) and Detractors (scores 6 and below) and then calculates % of Promoters - % of Detractors.  
...
iglite %>%
  ...
  transmute(
         Month = mth,
         `Total Reviews` = favorable + neutral + unfavorable,
         `% Favorable` = favorable/`Total Reviews`,
         `% Neutral` = neutral/`Total Reviews`,
         `% Unfavorable` = unfavorable/`Total Reviews`,
         `NPS` = `% Favorable` - `% Unfavorable`
         ) %>%
  mutate(across(c(starts_with('%'), 'NPS'), ~percent(.x, accuracy = .1))) %>%
  knitr::kable(
    align = 'c'
  )
```
...
With the EDA portion done, its on to Text Mining the reviews.  In a past-post I had used [the Tidytext Ecosystem to look at Tweet difference between Instagram and TikTok](https://jlaw.netlify.app/2020/08/02/what-s-the-difference-between-instagram-and-tiktok-using-word-embeddings-to-find-out/) but this time I will be using the [Bnosac](http://www.bnosac.be) ecosystem of packages to do Biterm Modeling, Sentiment Analysis with dependency parsing, and then the `textrank` and `wordcloud` package to generate a word-cloud of extracted keywords.

### Pre-processing with udpipe

In [prior text-mining posts](https://jlaw.netlify.app/2020/10/07/looking-for-media-bias-in-coverage-of-trump-s-covid-diagnosis/), I used tidytext to handle tokenization, however, in this analysis I will leverage the `udpipe` package.  The `udpipe` is a R wrapper around the C++ library of the same name that uses a pre-trained language models to [easily tokenize, tag, lemmatize or perform dependency parsing on text in any language](https://bnosac.github.io/udpipe/docs/doc1.html).  The "ud" in udpipe stands for Universal Dependencies which is a "[framework for consistent annotation of grammar](https://universaldependencies.org/#ud-treebanks)".
...
```{r echo = F}
cleaned %>% slice(1) %>% select(text) %>% knitr::kable()
```
...
The first analysis task will be biterm modeling using the `BTM` package.  The Biterm Topic Model model was developed by [Yan et. al](https://github.com/xiaohuiyan/xiaohuiyan.github.io/blob/master/paper/BTM-WWW13.pdf) as a means to determining the topics that occur in short-texts such as Tweets (or in this case Google Play Reviews).  Its meant to provide an improvement to traditional topics modeling in uses cases such as this.  My understanding of the difference between traditional topic modeling and biterm topic model is that in the former, the model learns word co-occurrence within documents, while with the later, the model learns word co-occurrences within a window across the entire set of documents.  In this context a "biterm" [consists of two words co-occurring in the same context, for example, in the same short text window](https://github.com/bnosac/BTM).  This analysis is modeled after the one from [bnosac](http://www.bnosac.be/index.php/blog/98-biterm-topic-modelling-for-short-texts).
...
```{r BTM_Vis, fig.width=9, fig.height=9}
library(textplot)
library(ggraph)
set.seed(123456)
...
This next piece is based on a [bnosac blog post](http://www.bnosac.be/index.php/blog/85-you-did-a-sentiment-analysis-with-tidytext-but-you-forgot-to-do-dependency-parsing-to-answer-why-is-something-positive-negative) and will leverage the dependency output from `udpipe` to see what words are connected to the words with negative sentiment.
...
For these lists I will get the data used in the `sentometrics` [package](https://github.com/SentometricsResearch/sentometrics):
