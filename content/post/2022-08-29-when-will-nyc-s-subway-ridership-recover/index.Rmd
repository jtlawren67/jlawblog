...
During the writing of my posts about COVID's effect on the NYC Subway ridership the New York Times published an article called [*The Pandemic Wasn’t Supposed to Hurt New York Transit This Much*](https://www.nytimes.com/2022/08/15/nyregion/mta-nyc-budget.html).  In this article there's comment that:
...
The data is the same set from my [initial post](post](https://jlaw.netlify.app/2022/07/13/how-much-has-covid-cost-the-nyc-subway-system-in-lost-fares/).  Its initially at the week/station/fare level.  For this exercise I just need the data at the weekly level.
...
My [first blog post](https://jlaw.netlify.app/2022/07/13/how-much-has-covid-cost-the-nyc-subway-system-in-lost-fares/) in this series covered the `modeltime` package in more detail for trying out many different forcasting models.  That post found that Prophet with XGBoosted Errors was the best model.  Here I'll be replicating that workflow for that modeltype.

*Step 1: Defining the pre-processing recipe*

This defines the forecasting formula as predicting fares based on all other features and then creates a bunch of time series specific features from the date field in `step_timeseries_signature`.  `step_rm` removes some variables created in the prior step that isn't useful, and finally `step_dummy` turns all the categorical variables into one-hot encoded indicators.  Here I also set the training data set as the MTA Fares beginning during the COVID period (after April 1, 2020) since training on the prior time period will give very strange results.
...
Sticking with the `tidymodels` framework, here I define a workflow which will consist of the recipe created in *Step 1* through `add_recipe` and the model set through `add_model()`.  Within `add_model()` sets the model type to be Boosted Prophet.  I believe the 'prophet_xgboost' is the default engine so `set_engine` isn't necessary, but good to keep around anyway.
...
In this instance I don't have a test set to work with so I'm jumping directly into forecasting.  Also since I don't know how long it will take for the forecast to recover to pre-COVID levels, I'll set the forecast horizon for 6 years. I provide the actual data in the `actual_data` field so that its included in the output data set.
...
The `modeltime` package makes it easy to visualize the data through the `plot_modeltime_forecast` package.  The default is to create a `plot.ly` plot but that can be coverted to a `ggplot2` plot by setting `.interactive` to `FALSE`.
...
3. For each `run_id_80` get the sum of `above_80_inds` to represent the length of each run
...
Now I can plot the recovery percentage by week and show that the first time there are four concecutive weeks above 80% is `r rec_pct[which.max(rec_pct$run_length_80 >= 4), ]$week_start` and the first time there are four concecutive weeks above 100% of the Pre-COVID value is `r rec_pct[which.max(rec_pct$run_length_100 >= 4), ]$week_start`.
