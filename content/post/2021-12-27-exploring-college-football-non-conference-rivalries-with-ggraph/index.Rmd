...
In order to use the {tidygraph} package the edge list needs to have *from* and *to* columns even in the graph is undirected.  Then once I have the edge list I construct a *weight* column by using the `count()` function from {dplyr}.
...
ggraph(lay) + 
  ...
  geom_edge_arc(strength = 0.4, 
                aes(edge_alpha = weight, 
                    edge_color = weight, 
                    edge_width = weight,
                    label = if_else(weight >= 8, paste0(first_team,'-',second_team), "")
                ),
                vjust = -0.5,
                hjust = 1,
                label_colour = 'white',
                label_size = 5) + 
  scale_edge_colour_viridis(begin = .2, end = .8, option = "A", direction = 1, guide = 'none') + 
  scale_edge_width_continuous(range = c(.5, 2), guide = 'none') + 
  scale_edge_alpha_continuous(guide = 'none', range = c(0.3, 1)) + 
  ...
  theme(
    ...
    plot.subtitle = element_textbox_simple(family = 'roboto', size = 20, color = 'white'),
    plot.title = element_markdown(hjust = .5, family = 'roboto', color = 'white', size = 25),
  )
...
While are not terrible surprising for anyone that follows college football, this post hopefully shows how you can create a network graph out of geographic coordinates and fix the layout so that it can be applied on top of a real map.  
